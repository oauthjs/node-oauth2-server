...
describe('Grant', function() {
  ...
  describe('when parsing request', function () {
    ...
    it('should ensure grant_type is allowed', function (done) {
      var app = bootstrap({ model: model, grants: ['refresh_token'] });
          ...
          model.saveAuthorizedClientIds('password', client.clientId, function(err, ok) {
            if (err) {
              ...
              request(app)
                  .post('/oauth/token')
                  .set('Content-Type', 'application/x-www-form-urlencoded')
                  .send({grant_type: 'password', client_id: client.clientId, client_secret: client.clientSecret})
                  .expect(400, /Missing parameters. \\"username\\" and \\"password\\" are required/i, done);
            }
          });
        }
      });
          ...
          model.saveAuthorizedClientIds('password', client.clientId, function (err, ok) {
            if (err) {
              ...
              request(app)
                  ...
                  .set('Authorization', 'Basic ' + new Buffer(client.clientId + ":" + client.clientSecret).toString('base64'))
        ...
        if (err) {
          ...
          model.saveAuthorizedClientIds('password', client.clientId, function (err, ok) {
            if (err) {
              ...
              var user = {};
              user.id = "username" + new Date().getTime();
