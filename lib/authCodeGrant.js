...
function checkParams (done) {
  ...
  if (this.config.authorizedParameters.length) {
    var parts = url.parse(this.redirectUri, true);
    delete parts.search;
    this.config.validParameters = {};
    var self = this;
    this.config.authorizedParameters.map(function (param) {
      self.config.validParameters[param] = parts.query[param];
      delete parts.query[param];
    });
    this.redirectUri = url.format(parts);
  }
...
function redirect (done) {
  var parts = url.parse(this.client.redirectUri);
  delete parts.search;
  parts.query = this.config.validParameters || {};
  parts.query.code = this.authCode;

  this.res.redirect(url.format(parts));
