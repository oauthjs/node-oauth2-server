...
function checkParams (done) {
  ...
  if(this.config.requireAuthGrantCredentials) {
    this.client = credsFromBasic(this.req);
    if (!this.client && this.req.body) {
      if(this.req.body) {
        this.client = credsFromBody(this.req) || {};
      } else {
        this.client = {};
      }
    }
...
function credsFromBasic (req) {
  ...
  return {clientId: user.name, clientSecret: user.pass};
}
...
function credsFromBody (req) {
  if ((req.body.client_id || req.query.client_id) && (req.body.client_secret || req.query.client_secret)) {
    return {clientId: req.body.client_id || req.query.client_id, clientSecret: req.body.client_secret || req.query.client_secret};
  } else {
    return false;
  }
}
...
function checkClient (done) {
  ...
  this.model.getClient(self.client.clientId || self.clientId, self.client.clientSecret || null, function (err, client) {
    console.log(err);
