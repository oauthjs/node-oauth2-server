...
function AuthCodeGrant(config, req, res, next, check) {
  ...
  this.res = res;
  this.check = check;
  this.responseType = this.req.query.response_type || this.req.body.response_type;
  var self = this;
...
function redirect (done) {
  if(this.responseType === 'code') {
    this.res.redirect(this.client.redirectUri +
      (this.client.redirectUri.indexOf('?') != -1 ? '&' : '?') + 'code=' +
      this.authCode + (this.req.query.state ? '&state=' + this.req.query.state : ''));
  } else {
    this.res.redirect(this.client.redirectUri + '#access_token=' + this.accessToken +
        (this.req.query.state ? '&state=' + this.req.query.state : '') + '&token_type=Bearer');
  }
