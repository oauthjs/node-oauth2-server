...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client) {
  ...
  return Promise.try(function () { return model.getRefreshToken(request.body.refresh_token, context); })
    .then(function(token) {
      if (!token) {
...
RefreshTokenGrantType.prototype.revokeToken = function(token) {
  var model = this.model, context = this.context;
  return Promise.try(function () { return model.revokeToken(token, context); })
    .then(function(token) {
      if (!token) {
        throw new InvalidGrantError('Invalid grant: refresh token is invalid');
      }
...
RefreshTokenGrantType.prototype.saveToken = function(user, client, scope) {
  ...
  return Promise.all(fns)
    ...
    .spread(function(accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      return this.model.saveToken(token, client, user, this.context);
    });
};
