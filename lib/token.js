...
token.handle = function (req, res, next) {
  ...
  // Grant type
  req.oauth.grant_type = req.body && req.body.grant_type;
  if (!req.oauth.grant_type || !req.oauth.grant_type.match(this.regex.grantType)) {
  ...
  if (!creds.client_id || !creds.client_id.match(this.regex.client_id)) {
  ...
  this.model.getClient(creds.client_id, creds.client_secret, function (err, client) {
    ...
    oauth.model.grantTypeAllowed(client.client_id, req.oauth.grant_type,
...
token.grant = function (req, res, next) {
  ...
  if (req.oauth.grant_type.match(/^http(s|):\/\//) && this.model.extendedGrant) {
...
token.grantAccessToken = function (req, res, next) {
  ...
  var createRefreshToken = function (err, refreshToken) {
    ...
    var data = {
      refresh_token: req.oauth.accessToken.refresh_token,
      client_id: req.oauth.client.client_id,
      user_id: req.user.id,
      expires: expires
    };

    oauth.model.saveRefreshToken(data, function (err) {
      if (err) return next(error('server_error', false, err));
  ...
  var createAccessToken = function (err, accessToken) {
    ...
    var data = {
      access_token: req.oauth.accessToken.access_token,
      client_id: req.oauth.client.client_id,
      user_id: req.user.id,
      expires: expires
    };

    oauth.model.saveAccessToken(data, function (err) {
      if (err) return next(error('server_error', false, err));
      issueRefreshToken();
    });
  };
