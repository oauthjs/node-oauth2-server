...
function checkClient (done) {
  if (skipCheckClientGrantType.indexOf(this.grantType) > -1) return done();
...
function checkGrantType (done) {
  ...
  switch (this.grantType) {
    ...
    case 'implicit':
      return useImplicitGrant.call(this, done);
    default:
...
/**
 * Grant for implicit grant type
 *
 * @param  {Function} done
 */
function useImplicitGrant (done) {
    // Client credentials
    var clientId = this.client.clientId;
    ...
    var self = this;
    return this.model.getUserFromClient(clientId,
        function (err, user) {
            if (err) return done(error('server_error', false, err));
            if (!user) {
                return done(error('invalid_grant', 'Client credentials are invalid'));
