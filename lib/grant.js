...
function extractCredentials (done) {
  ...
  if (this.client) {
    ...
    } else if (!this.client.clientSecret && !this.config.allowPublicClients) {
      return done(error('invalid_client', 'Missing client_secret parameter'));
    }
  } else if (!this.config.allowPublicClients) {
    return done(error('invalid_client', 'Missing client credentials'));
  }
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || (self.client &&
          refreshToken.clientId !== self.client.clientId) ||
        (!self.client && !self.config.allowPublicClients)) {
...
function checkGrantTypeAllowed (done) {
  this.model.grantTypeAllowed((this.client ? this.client.clientId : null),
      this.grantType,
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(accessToken,
      (this.client ? this.client.clientId : null), expires,
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(refreshToken,
      (this.client ? this.client.clientId : null), expires,
      this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
