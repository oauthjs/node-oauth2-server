...
function extractCredentials (done) {
  ...
  if (this.req.method !== 'POST' ||
    ...
    return done(errorManager.error('invalid_request',
  ...
  if (!this.client.clientId ||
    ...
    return done(errorManager.error('invalid_client',
      'Invalid or missing client_id parameter'));
  } else if (!this.client.clientSecret) {
    return done(errorManager.error('invalid_client', 'Missing client_secret parameter'));
  }
...
function checkClient (done) {
  ...
  this.model.getClient(this.client.clientId, this.client.clientSecret,
      function (err, client) {
    if (err) return done(errorManager.error('server_error', false, err));

    if (!client) {
      return done(errorManager.error('invalid_client', 'Client credentials are invalid'));
    }
...
function checkGrantType (done) {
  ...
  switch (this.grantType) {
    ...
    default:
      done(errorManager.error('invalid_request',
...
function useAuthCodeGrant (done) {
  ...
  if (!code) {
    return done(errorManager.error('invalid_request', 'No "code" parameter'));
  }
  ...
  this.model.getAuthCode(code, function (err, authCode) {
    ...
    if (!authCode || authCode.clientId !== self.client.clientId) {
      return done(errorManager.error('invalid_grant', 'Invalid code'));
    } else if (authCode.expires < self.now) {
      return done(errorManager.error('invalid_grant', 'Code has expired'));
    }

    self.user = authCode.user || { id: authCode.userId };
    if (!self.user.id) {
      return done(errorManager.error('server_error', false,
...
function usePasswordGrant (done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(errorManager.error('server_error', false, err));
    if (!user) {
      return done(errorManager.error('invalid_grant', 'User credentials are invalid'));
    }
...
function useRefreshTokenGrant (done) {
  ...
  if (!token) {
    return done(errorManager.error('invalid_request', 'No "refresh_token" parameter'));
  }
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
      return done(errorManager.error('invalid_grant', 'Invalid refresh token'));
    } else if (refreshToken.expires !== null &&
        refreshToken.expires < self.now) {
      return done(errorManager.error('invalid_grant', 'Refresh token has expired'));
    }
...
function useClientCredentialsGrant (done) {
  ...
  return this.model.getUserFromClient(clientId, clientSecret,
    ...
    if (err) return done(errorManager.error('server_error', false, err));
    if (!user) {
      return done(errorManager.error('invalid_grant', 'Client credentials are invalid'));
    }
...
function useExtendedGrant (done) {
  ...
  this.model.extendedGrant(this.grantType, this.req,
    ...
    if (err) {
      return done(errorManager.error(err.error || 'server_error',
    ...
    if (!supported) {
      ...
      return done(errorManager.error('invalid_request', 'Invalid request.'));
    }
