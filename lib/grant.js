...
function useClientCredentialsGrant (done) {
  ...
  // Get username
  var uname = this.req.body.username || this.req.query.username;

  if (!uname) {
    return done(error('invalid_client',
      'Missing parameters. "username" is required'));
  }

  var self = this;
  return this.model.getUserFromClient(clientId, clientSecret, uname, function (err, user) {
    if (err) return done(error('server_error', false, err));
