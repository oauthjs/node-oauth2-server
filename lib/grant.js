...
module.exports = (function () {
   ...
   * Grant param
   *
   * @object {Object}   config Instance of OAuth object
   * @param {Object}   req
   * @param {Object}   res
   * @param {Function} next
   */
  var Grant = function (config, req, res, next) {
    this.config = config;
   ...
   * @public
   */
  Grant.prototype.run = function (next) {
    var _this = this;
    (function run(pos) {
      if (pos < len) {
        fns[pos].call(_this, function (err) {
   ...
   */
  var credsFromBasic = function (req) {
   ...
   * @return {Object} Client
   */
  var credsFromBody = function (req) {
   ...
   * @param  {Function} done
   */
  var usePasswordGrant = function (done) {
    // User credentials
   ...
   * @param  {Function} done
   */
  var useRefreshTokenGrant = function (done) {
    ...
    this.model.getRefreshToken(token, function (err, refreshToken) {
      ...
      if (!refreshToken || refreshToken.client_id !== _this.client.client_id) {
        return done(error('invalid_grant', 'Invalid refresh token'));
      } else if (refreshToken.expires !== null &&
          refreshToken.expires < _this.config.now) {
      ...
      if (_this.model.revokeRefreshToken) {
        return _this.model.revokeRefreshToken(token, function (err) {
          if (err) return done(error('server_error', false, err));
          done();
        });
      }

      _this.user = { id: refreshToken.user_id };
      done();
    });
  };
   ...
   * @param  {Function} done
   */
  var useExtendedGrant = function (done) {
    var _this = this;
    this.model.extendedGrant(this.req, function (err, supported, user) {
      if (err && err.error && err.description) {
        return done(error(err.error, err.description));
      }
   ...
   * @param  {Function} callback
   */
  var tokenGenerator = function (type, callback) {
    if (this.model.generateToken) {
      this.model.generateToken(type, this.req, function (err, generatedToken) {
        if (err) return callback(error('server_error', false, err));
        if (!generateToken) return generateToken(callback);
  ...
  function saveAccessToken (done) {
    ...
    if (this.config.accessTokenLifetime !== null) {
      expires = new Date(this.config.now);
