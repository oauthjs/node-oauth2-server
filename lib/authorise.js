...
/**
 * Check if a url is public
 *
 * @param {Object}   config Instance of OAuth object
 * @param {Object}   req
 */
function isPublicUrl(config, req){
    if(!config.publicUrlRegex){
        return false;
    }
    var publicUrlRegex = config.publicUrlRegex[req.method];
    
    if(!publicUrlRegex){
        return false;
    }
    
    for (var i = 0; i < publicUrlRegex.length; i++ ){
        if (new RegExp(publicUrlRegex[i]).test(req.originalUrl)){
            return true;
        }
    }
    return false;
