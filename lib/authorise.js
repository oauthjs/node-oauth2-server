 ...
 * Check scope
 *
 * Passes request information to the model for scope validation
 * @param  {Function} done
 * @this   OAuth
 */
function checkScope (done) {
  this.model.checkScope(this.bearerToken, this.req, function (err) {
    if (err) return done(error('invalid_scope', err));
