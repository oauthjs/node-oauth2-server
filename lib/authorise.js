...
function getBearerToken (done) {
  ...
  if (postToken) {
    ...
    if (!this.req.is('application/x-www-form-urlencoded')) {
      return done(error('invalid_request', 'When putting the token in the body, ' +
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    if (token.expires !== null &&
      (!token.expires || token.expires < _this.config.now)) {
      return done(error('invalid_token',
        'The access token provided has expired.'));
    }

    // Expose params
    _this.req.oauth.token = token;
    _this.req.user = token.user ? token.user : { id: token.user_id };
