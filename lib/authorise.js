...
var errorManager = require('./error'),
...
function getBearerToken (done) {
  ...
  if (methodsUsed > 1) {
    ...
    return done(errorManager.error('invalid_request', 'The access token was not found'));
  }
  ...
  if (headerToken) {
    ...
    if (!matches) {
      return done(errorManager.error('invalid_request', 'Malformed auth header'));
    }
  ...
  if (postToken) {
    ...
    if (!this.req.is('application/x-www-form-urlencoded')) {
      return done(errorManager.error('invalid_request', 'When putting the token in the ' +
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    if (!token) {
      return done(errorManager.error('invalid_token',
