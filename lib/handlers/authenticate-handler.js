...
AuthenticateHandler.prototype.getAccessToken = function(token) {
  ...
  return Promise.try(function () { return model.getAccessToken(token, context); })
...
AuthenticateHandler.prototype.validateScope = function(accessToken) {
  var model = this.model, context = this.context, scope = this.scope;
  return Promise.try(function () { return model.validateScope(accessToken, scope, context); })
    .then(function (scope) {
      if (!scope) {
